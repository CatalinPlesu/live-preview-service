...
func (r *RabbitMQ) PublishMessage(queueName string, message model.Message) error {
...
func (r *RabbitMQ) ConsumeMessages(queueName string, handler func(model.Message)) error {
	// Declare the queue if it doesn't exist
	_, err := r.Channel.QueueDeclare(
		queueName,
		true,  // Durable
